...
function BloomEffect() {
  ...
  useEffect(() => {
    ...
    const bloom = new UnrealBloomPass(new THREE.Vector2(size.width, size.height), 1.2, 0.55, 0.08);
...
function generateBrainNodes(count: number) {
  ...
  while (nodes.length < count) {
    ...
    const bump = 0.09 * Math.sin(4 * u + 3 * v) + 0.06 * Math.sin(7 * v - 2 * u) + 0.04 * Math.sin(5 * u * v);
...
function BrainNetwork({ colors, dissolving = false }: { colors: ModeColorSet; dissolving?: boolean }) {
  ...
  useFrame(({ clock }) => {
    ...
    const dissolveEase =
      dissolveProgress < 0.6
        ? dissolveProgress * dissolveProgress * 0.4
        : 0.144 + Math.pow((dissolveProgress - 0.6) / 0.4, 3) * 0.856;
    ...
    if (groupRef.current) {
      ...
      const dissolveScale = 1 - dissolveEase * 0.98;
      groupRef.current.scale.setScalar(Math.max(0.001, breath * formScale * dissolveScale));
      groupRef.current.position.z = dissolveEase * 2.5;
      groupRef.current.position.x = Math.sin(dissolveEase * Math.PI * 4) * dissolveEase * 0.3;
      groupRef.current.position.y = Math.cos(dissolveEase * Math.PI * 3) * dissolveEase * 0.2;
    }
  ...
  return (
    <group ref={groupRef}>
      <lineSegments>
        <bufferGeometry ref={lineGeomRef}>
          <bufferAttribute
            ...
            itemSize={3}
          />
          <bufferAttribute attach="attributes-color" count={edgePositions.length / 3} array={edgeColors} itemSize={3} />
        </bufferGeometry>
        <lineBasicMaterial
          vertexColors
          transparent
      ...
      {pulses
        .filter((p) => edges[p.edgeIdx] && nodes[p.toNode])
        .map((p) => {
...
function BackgroundStars() {
  ...
  type Streak = { line: THREE.Line; velocity: THREE.Vector3; life: number; maxLife: number; head: THREE.Vector3 };
  ...
  const spawnStreak = useCallback(() => {
    ...
    streaksPool.current.push({ line, velocity: dir, life: 0, maxLife: 2.5, head });
  }, []);
  ...
  useFrame((_, delta) => {
    ...
    for (const s of streaksPool.current) {
      ...
      pa.needsUpdate = true;
      const outOfBounds =
        Math.abs(s.head.x) > BOUNDS.x + 2 || Math.abs(s.head.y) > BOUNDS.y + 2 || Math.abs(s.head.z) > BOUNDS.z + 2;
